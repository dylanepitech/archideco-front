        ...
        <div className="min-h-screen flex flex-col">
            ...
            <main className="flex-grow container mx-auto px-4 py-8">
                ...
                {wishlistItems.length === 0 ? (
                    <p className="text-center text-gray-600">Votre liste de souhaits est vide.</p>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {wishlistItems.map(item => (
                            <div key={item.id} className="border rounded-lg p-4 shadow-sm relative">
                                <Link to={`/${item.categoryTitle}/${item.title}/${item.id}`} className="block">

                                    {Object.keys(item.images).length > 0 && (
                                        <img
                                            src={removeBaseUrl(item.images[Object.keys(item.images)[0]][0].image)} // Récupérer la première image du premier tableau
                                            alt={item.title}
                                ...
                                <div className="flex justify-between items-center">
                                    <button
                                        ...
                                        onClick={() => cart ? handleUpdateCart(item?.id) : handleCreateCart(item?.id)}
                                        className="bg-green-emerald text-white px-4 py-2 rounded hover:bg-green-duck"
                                    >
                                        Ajouter au panier
                                    </button>
                                    <button

...
export default function ProductListPage() {
  ...
  const applyFilters = () => {
    ...
    if (filters.categories.length > 0) {
      filtered = filtered.filter(product => filters.categories.includes(product.categoryTitle));
    }
    ...
    if (filters.weight) {
      filtered = filtered.filter(product => parseFloat(product.weight) <= parseFloat(filters.weight));
    }
  ...
  const currentProducts = filteredProducts.length <= productsPerPage
    ? filteredProducts
            ...
            <div className="flex justify-center mt-4">
              ...
              {[...Array(totalPages).keys()].map((pageNumber) => (
                <button
                  ...
                  className={`px-4 py-2 ${currentPage === pageNumber + 1 ? 'bg-blue-500 text-white' : 'bg-gray-300 text-black'} rounded-none hover:bg-gray-400`}
                >
                  {pageNumber + 1}
                </button>
              ))}
